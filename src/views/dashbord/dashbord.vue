<template>
        <div class="dashbord">
            <div class="hotel floor">
                <h1>优惠酒店</h1>
                <div class="citys">
                    <button v-for="item in citys" :key="item.id" @click="cityChange(item.id)" :class="active === item.id ? 'active' : ''">
                        {{item.name}}
                    </button>
                </div>
            ...
            <div class="travel floor">
                <h1>坐火车去旅行</h1>
                <div class="cityList">
                    <card v-for="item in travelCitys" :img="item.img" :key="item.id" :imgTitle="item.city" :imgscript="item.describe" :show-img-text="true">
                    </card>
                </div>
            </div>
